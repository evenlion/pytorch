...
def forward(self, x, y):
    ...
    def test_split_const_gm_with_lifted_constants(self):
        ...
        for n in new_gm.graph.nodes:
            if n.op == "placeholder":
                counter += 1
        # expect self.b, x and a folded_const
        self.assertTrue(counter == 3)
        # return (self.b, folded_const)
        const_folded_value = const_gm(*lifted_constant_values)

        test_input = torch.randn(4, 4)
        # new_gm(self.b, x, folded_const)
        actual = new_gm(const_folded_value[0], test_input, const_folded_value[1])[0]
        expected = mod(test_input)
        self.assertEqual(actual, expected)
        const_gm, _ = split_const_gm(ep.graph_module, lifted_constant_names, lambda x: True)
        counter = 0
        for node in const_gm.graph.nodes:
            if node.op == "call_function":
